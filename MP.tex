...
\def \BookNameHead {Aplikace pro výuku algoritmů}
\def \Author {David Martínek} \def \AuthorClass{IT4}
\def \Year {2018}
\def \StudyFieldNumber {18-20-M/01}
\def \StudyField {Informační technologie -- správa sítí a programování}
\def \Supervisor {Ing.~Jiří Roubal}
\def \APforPrint {1}

\def \BookName {MATURITNÍ PRÁCE -- PROFILOVÁ ČÁST MATURITNÍ ZKOUŠKY}
\def \HEAD {Vyšší odborná škola, Střední škola, Centrum odborné přípravy\\[1.5ex] Sezimovo Ústí}
\def \SCHOOL {Vyšší odborná škola, Střední škola,}
\def \FACULTY {Centrum odborné přípravy, Sezimovo Ústí}
...
%-----<<< DECLARATION >>>-----
$ $
\input{\cestaFiles DeclarMP.tex}        %input file
\newpage
%-----<<< ----------- >>>-----
...
%-----<<< ANNOTATION >>>-----
\input{\cestaFiles AnnotationMP.tex}    %input file
\cleardoublepage
%-----<<< ---------- >>>-----
...
\pagestyle{fancy} \fancyhf{} \lhead{Maturitní práce~--~\BookNameHead~--~\Author,~\AuthorClass} \rhead{\Year} \cfoot{\thepage}
